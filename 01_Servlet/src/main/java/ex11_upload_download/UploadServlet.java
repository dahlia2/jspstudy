...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		  ...
		  3. 압축 해제 후 lib 디렉터리에 있는 cos.jar 파일을 프로젝트의 lib로 등록한다. (WEB-INF > lib)
		*/
		...
		*/
		...
		MultipartRequest multipartRequest = new MultipartRequest(
				...
				realPath,  			// 업로드 경로 전달 (String 문자열)
				1024 * 1024 * 10,   // 최대 크기 
				"UTF-8",			// 인코딩
				new DefaultFileRenamePolicy() // 파일명 중복 처리 정책 (파일명 뒤에 넘버링) // Default.. 클래스 = 디폴트로 설정 (방법 설정도 가능)
		);
		...
		out.println("<ul style=\"list-style-type: circle; font-size: 24px;\">");  // <ul> 태그에 css 적용
