...
/*
 	URL
 	1. 구성
 		프로토컬://호스트:포트번호/ContentPath/URLMapping
 	2. HelloServlet의 URL
   		http://localhost:9090/01_Servlet/HelloServlet
*/

@WebServlet("/HelloServlet")  // URLMapping(클래스 이름)
...
public class HelloServlet extends HttpServlet {
	...
	*/
	...
	/*
		 ...
		 3) 클라이언트의 요청(HttpServletRequest)을 직접 처리할 수 있다.
	...
	*/
	...
	/*
	  4. doGet()
	  	 ...
	  	 2) GET 방식의 요청의 예시
	  	 	(1) <a href="http://localhost:9090/01_Servlet/HelloServlet">       // a링크면 100% get방식
	  	 	(2) <form action="http://localhost:9090/01_Servlet/HelloServlet">
	  	 	(3) location.href="http://localhost:9090/01_Servlet/HelloServlet"  // 메소드 생략하면 get 방식
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// getContextPath 와 같이 Served at: 글씨를 사용자에게 보내주겠다(response)
	}
	...
	/*
	  5. doPost()
	  	...
	  	2) POST 방식의 요청의 예시
	  		(1) <form action="http://localhost:9090/01_Servlet/HelloServlet"> 
	  		(2) $.ajax({
		  	 		type: 'POST',
		  	 		url: 'http://localhost:9090/01_Servlet/HelloServlet'
	  	 		})
