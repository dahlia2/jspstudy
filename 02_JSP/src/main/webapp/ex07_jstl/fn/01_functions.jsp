...
<body>
	...
	<h3>${fn:substringBefore(str, ' ')}</h3>  <!-- W 이전의 텍스트 -->
	...
	<c:if test="${fn:containsIgnoreCase(str, 'h')}"> <!-- containsIgnoreCase : 대소문자 구분 x  -->
		<h3>H, h를 포함한다.</h3>
	</c:if>
	
	<c:set var="str3" value="a,b,c,d,e,f,g,h,i,j,k" scope="page" />
	<c:set var="items" value="${fn:split(str3, ',')}" scope="page" />  <!-- 배열 -->
