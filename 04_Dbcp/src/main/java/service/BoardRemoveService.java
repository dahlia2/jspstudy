...
public class BoardRemoveService implements IBoardService {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
		...
		int board_no = Integer.parseInt(strBoard_no.isEmpty()? "0" : strBoard_no);  // 매개변수가 없을 때 0 처리 (실패) 
					// form 요소는 null 처리가 될 수 없음. 즉 if(strBoard_no = null)문을 대신하여 imEmpty()을 활용한 것
